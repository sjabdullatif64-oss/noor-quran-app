...
public class SpeechRecognition extends Plugin implements Constants {
    ...
    public void load() {
        ...
        //
        // Calling SpeechRecognizer.createSpeechRecognizer() via
        // bridge.getWebView().post() blocks the Android main/UI thread while
        // the system binds to the Google speech recognition service (IPC).
        // On Android 13+ this binding can take several seconds.  The WebView
        // JavaScript engine also runs on the main thread, so while that
        // binding is in progress:
        //   • bridge.eval() posts (used by call.resolve()) are queued but
        //     never executed
        //   • JS setTimeout / Promise microtasks never fire
        //   • Every subsequent Capacitor bridge call (e.g. checkPermissions)
        //     appears to hang with no timeout fallback possible
        //
        // beginListening() already creates/recreates the recognizer on demand,
        // so eager creation here is both unnecessary and harmful.
        Logger.info(getLogTag(), "load() complete — SpeechRecognizer created lazily in beginListening()");
    }

    /**
     * Check RECORD_AUDIO permission without prompting.
     *
     * Overrides the Capacitor base-class implementation for two reasons:
     *
     * 1. The Capacitor 8 base Plugin.checkPermissions() is annotated with both
     *    @PluginMethod and @PermissionCallback.  The @PermissionCallback
     *    annotation causes initializeActivityLaunchers() to register the method
     *    as a permission-result launcher, which can confuse Capacitor 8's
     *    bridge dispatch in edge cases.  Our override carries only @PluginMethod
     *    so it is treated as a plain, directly-callable method.
     *
     * 2. Using ActivityCompat.checkSelfPermission() directly is faster and
     *    cannot block — unlike the base-class path through
     *    bridge.getPermissionStates() which reads SharedPreferences.
     */
    @PluginMethod
    @Override
    public void checkPermissions(PluginCall pluginCall) {
        ...
        if (status == PackageManager.PERMISSION_GRANTED) {
            result.put(SPEECH_RECOGNITION, PermissionState.GRANTED.toString());
        } else {
            // Could be first-time (prompt) or permanently denied.
            // Return PROMPT here; requestPermissions() will determine the
            // actual state and the JS flow handles denial from its result.
            result.put(SPEECH_RECOGNITION, PermissionState.PROMPT.toString());
        }
