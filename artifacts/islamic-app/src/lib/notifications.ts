...
// Default: only Daily Quran Ayah is ON — everything else is opt-in
export const DEFAULT_SETTINGS: AllNotifSettings = {
  quranAyah:       { enabled: true,  time: "08:00" }, // ON by default for new users
...
export function getNotifSettings(): AllNotifSettings {
  try {
    ...
    // Deep-merge: fill any missing keys from DEFAULT_SETTINGS
