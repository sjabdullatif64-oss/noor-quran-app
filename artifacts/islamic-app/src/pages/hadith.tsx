...
const TOPIC_COLORS: Record<string, { text: string; bg: string }> = {
  ...
  "Knowledge":   { text: "text-blue-300",    bg: "rgba(96,165,250,0.15)"  },
   "Quran":       { text: "text-primary", bg: "rgba(52,211,153,0.15)"  },
  ...
  "Purity":      { text: "text-cyan-300",    bg: "rgba(34,211,238,0.15)"  },
   "Taqwa":       { text: "text-primary", bg: "rgba(52,211,153,0.18)"  },
  ...
  "Patience":    { text: "text-rose-300",    bg: "rgba(244,63,94,0.15)"   },
   "Salawat":     { text: "text-primary",   bg: "rgba(251,191,36,0.15)"  },
};

function topicStyle(topic: string) {
   return TOPIC_COLORS[topic] ?? { text: "text-primary", bg: "rgba(52,211,153,0.12)" };
}
    ...
    >
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-muted-foreground text-xs">#{hadith.id}</span>
        </div>

        {/* Arabic text */}
        <p className={`leading-relaxed text-right font-arabic ${large ? "text-2xl" : "text-xl"} text-foreground`}
        ...
        >
          <div>
            <p className="text-primary text-xs font-semibold">{hadith.source}</p>
            <p className="text-muted-foreground text-xs mt-0.5">Narrated by {hadith.narrator}</p>
          </div>
          {expanded
        ...
        {expanded && (
          <div className="rounded-2xl border border-border p-4 space-y-2 animate-in fade-in duration-200 bg-muted">
            <p className="text-primary text-xs font-semibold">Source Details</p>
            <p className="text-foreground text-sm">{hadith.source}</p>
            <p className="text-muted-foreground text-xs">Narrator: {hadith.narrator}</p>
            <p className="text-muted-foreground text-xs">Topic: {hadith.topic}</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="min-h-screen pb-28 md:pb-10 animate-in fade-in duration-500 bg-background">
      ...
      <div className="flex items-center gap-3 px-5 pt-6 pb-5">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-serif font-bold text-primary">Hadith of the Day</h1>
          <p className="text-muted-foreground text-xs mt-0.5">Authentic sayings of the Prophet ﷺ</p>
        </div>
        <button
          onClick={handleRandom}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium text-primary border border-border bg-card transition-all active:scale-95"
        >
      ...
      {/* Coin earned message */}
      {coinMsg && (
        <div className="mx-5 mb-3 flex items-center gap-2 px-4 py-2.5 rounded-2xl border border-border animate-in fade-in duration-200 bg-muted">
          <span className="text-base">🪙</span>
          <p className="text-primary text-sm font-semibold">+{COIN_EVENTS.hadith_read} coins earned!</p>
        </div>
      )}
      ...
      <div className="px-4 space-y-5">
        ...
        <div className="text-center py-4">
          <p className="text-2xl text-primary" style={{ fontFamily: "'Amiri', serif" }}>بِسْمِ اللهِ الرَّحْمٰنِ الرَّحِيمِ</p>
          <p className="text-muted-foreground text-xs mt-1">In the name of Allah, the Most Gracious, the Most Merciful</p>
        </div>
        ...
        <div>
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-3 rounded-2xl border border-border transition-all active:scale-[0.98] bg-card"
          >
            <span className="text-primary text-sm font-semibold">All Hadith ({HADITH_COLLECTION.length})</span>
