...
export function AzanSettings() {
  ...
  return (
    <div className="min-h-[100dvh] bg-background text-primary-foreground">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-background/90 backdrop-blur-sm border-b border-border">
        <div className="flex items-center gap-3 px-4 py-4">
          <Link href="/notifications">
            <button className="rounded-full p-2 hover:bg-muted/50 transition-colors" aria-label="Back">
              <ChevronLeft className="w-5 h-5" />
            </button>
          </Link>
          <div>
            <h1 className="text-lg font-bold">Azan Notifications</h1>
            <p className="text-xs text-muted-foreground">Automatic prayer-time Azan on your device</p>
          </div>
          {saving && <Loader2 className="w-4 h-4 ml-auto animate-spin text-primary" />}
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        {/* Master toggle */}
        <section className="rounded-2xl bg-muted/50 border border-border overflow-hidden">
          ...
          {settings.enabled && (
            <div className="border-t border-border px-5 py-3 flex items-center gap-2 bg-muted/30">
        ...
        {/* Per-prayer toggles */}
        <section>
          <h2 className="text-xs font-semibold text-muted-foreground/80 uppercase tracking-widest mb-3 px-1">
            Prayers
          </h2>
          <div className="rounded-2xl bg-muted/50 border border-border overflow-hidden divide-y divide-white/10">
              ...
              <div key={def.key} className="flex items-center justify-between px-5 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground/80">{def.sublabel}</p>
                  </div>
                </div>
                <Toggle
          ...
          {!settings.enabled && (
            <p className="text-xs text-muted-foreground/60 mt-2 px-1">Enable Azan above to configure individual prayers.</p>
          )}
        </section>
            ...
            {AZAN_SOUND_OPTIONS.map((opt) => {
              ...
              return (
                <button
                  ...
                  disabled={!isNative}
                  className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-muted/50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={[
                        "rounded-xl p-2.5",
                        active ? "bg-emerald-500/30" : "bg-muted/50",
                      ].join(" ")}
                    >
                      <Volume2
                        className={["w-4 h-4", active ? "text-primary" : "text-muted-foreground"].join(" ")}
                      />
                    </div>
                    <div>
                      <p className={["font-medium", active ? "text-primary" : ""].join(" ")}>
                        {opt.label}
                      </p>
                      <p className="text-xs text-muted-foreground/80">{opt.sublabel}</p>
                    </div>
                  </div>
          ...
          {isNative && (
            <>
              <button
                ...
                disabled={testing}
                className="w-full mt-3 flex items-center justify-center gap-2 rounded-2xl bg-primary hover:bg-primary/90 py-4 text-sm font-semibold text-primary-foreground transition-colors disabled:opacity-60"
              >
                {testing
        ...
        {isNative && settings.enabled && (
          <section>
            <button
              ...
              disabled={saving}
              className="w-full flex items-center justify-center gap-2 rounded-2xl bg-muted border border-border py-4 text-sm font-medium text-primary hover:bg-muted-foreground/20 transition-colors disabled:opacity-50"
            >
              {saving
                ? <Loader2 className="w-4 h-4 animate-spin" />
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground/80">View what happened the last time an Azan fired</p>
                </div>
              </div>
              {diagLoading
                ? <Loader2 className="w-4 h-4 animate-spin text-muted-foreground/80 shrink-0" />
                : <ChevronLeft className={["w-4 h-4 text-muted-foreground/80 shrink-0 transition-transform", diagOpen ? "rotate-90" : "-rotate-90"].join(" ")} />}
            </button>

            {diagOpen && (
              <div className="border-t border-border p-4 space-y-3">
                {diagLog ? (
                  <pre className="max-h-64 overflow-y-auto whitespace-pre-wrap break-words rounded-xl bg-muted/30 border border-border p-3 text-[10px] leading-relaxed text-emerald-200/90 font-mono">
                    {diagLog}
                  </pre>
                ) : (
                ...
                <div className="flex gap-2">
                  <button
                    onClick={loadDiagLog}
                    className="flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-muted/50 py-2.5 text-xs font-medium text-foreground hover:bg-white/15 transition-colors"
                  >
                  ...
                  <button
                    onClick={copyDiagLog}
                    disabled={!diagLog}
                    className="flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-muted/50 py-2.5 text-xs font-medium text-foreground hover:bg-white/15 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                  >
        ...
        {/* Info footer */}
        <section className="rounded-2xl bg-muted/50 border border-border p-4 text-xs text-muted-foreground/80 space-y-1.5">
