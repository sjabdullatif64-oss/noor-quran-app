...
import { Book, Bookmark, Compass, Home as HomeIcon, Moon, Sun } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col md:flex-row bg-background">
      ...
      <div className="md:hidden flex items-center justify-around px-2 py-2 border-t border-border bg-card sticky bottom-0 z-20">
        <MobileNavItem href="/" icon={<HomeIcon className="w-5 h-5" />} label="Home" active={location === "/"} />
        <MobileNavItem href="/quran" icon={<Book className="w-5 h-5" />} label="Quran" active={location === "/quran" || location.startsWith("/quran/")} />
        <MobileNavItem href="/prayer-times" icon={<Compass className="w-5 h-5" />} label="Prayers" active={location === "/prayer-times"} />
        <MobileNavItem href="/bookmarks" icon={<Bookmark className="w-5 h-5" />} label="Saved" active={location === "/bookmarks"} />
      </div>
    </div>
  );
}
